...
**CI integration.** No CI exists for the JS side either; adding a GitHub Actions workflow to run `make test` would be premature. Revisit when the project gets any other CI.
...
**Track `c-port/test/test_main` in git or add to `.gitignore`.** Already in `.gitignore` (added in `chore: untrack c-port binaries`).
